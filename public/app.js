...
function landPolys() {
  ...
  const D = Math.PI / 180;
  _landCache = WORLD_LAND_ENC.split(";").map((s) => {
    const v = s.split(",");
    const n = v.length / 2;
...
function drawGlobe(ctx, S, rot) {
  ...
  // ocean
  g = ctx.createRadialGradient(C - R * 0.45, C - R * 0.5, R * 0.1, C, C, R);
  g.addColorStop(0, "#74baf1");
  g.addColorStop(0.45, "#2679cb");
  g.addColorStop(1, "#093c72");
  ctx.fillStyle = g;
  ...
  // land — one shared radial gradient gives every continent spherical shading
  g = ctx.createRadialGradient(C - R * 0.45, C - R * 0.5, R * 0.05, C, C, R * 1.05);
  g.addColorStop(0, "#8ae7ab");
  g.addColorStop(0.5, "#33b268");
  g.addColorStop(1, "#0f6b46");
  ctx.fillStyle = g;
  ctx.strokeStyle = "rgba(4,50,30,.5)";
  for (const P of landPolys()) {
    let any = false;
    ctx.beginPath();
    for (let i = 0; i < P.n; i++) {
      const p = proj(P.lam[i], P.sp[i], P.cp[i]);
      let x = p[0], y = p[1];
      if (p[2] > 0) any = true;
      else {                                    // behind the horizon → pin to rim
    ...
    if (!any) continue;                         // fully on the far side
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  }
